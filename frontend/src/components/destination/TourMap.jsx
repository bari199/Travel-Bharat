...
const TourMap = ({ destination }) => {
  ...
  return (
    <section className="w-full bg-[#f8fafc] dark:bg-slate-950 py-12 sm:py-16 lg:py-20 relative overflow-hidden transition-colors duration-300">
      {/* Decorative blobs */}
      <div className="pointer-events-none absolute -top-20 -left-20 w-72 h-72 rounded-full bg-orange-100/40 dark:bg-orange-500/10 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 -right-16 w-64 h-64 rounded-full bg-blue-100/30 dark:bg-blue-500/10 blur-3xl" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <motion.h2
            variants={fadeUp}
            className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 dark:text-slate-100 leading-tight tracking-tight mb-3"
          >
        ...
        >
          {/* Toggle pills */}
          <div className="inline-flex items-center gap-1.5 mb-5 p-1.5 bg-white dark:bg-slate-900 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm">
            {[
                ...
                className={`relative flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300
                  ${
                    activeView === id
                      ...
                      : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-orange-400 hover:bg-slate-50 dark:hover:bg-slate-800"
                  }`}
              >
          ...
          {/* Panel */}
          <div className="relative w-full rounded-[26px] overflow-hidden border border-slate-200 dark:border-slate-700 shadow-lg shadow-slate-200/50 bg-white dark:bg-slate-900">
            <AnimatePresence mode="wait">
              {activeView === "map" ? (
                ...
                >
                  ...
                  />
                  {destination?.location && (
                    <div className="absolute top-4 left-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm rounded-xl px-3.5 py-2.5 shadow-lg border border-slate-100 dark:border-slate-800 flex items-center gap-2">
                      <span className="flex items-center justify-center w-6 h-6 rounded-full bg-orange-500 shrink-0">
                        <MapPin className="w-3 h-3 text-white" />
                      </span>
                      <span className="text-xs font-bold text-slate-700 dark:text-slate-300 max-w-[160px] truncate">
                        {destination.location}
                      </span>
                    </div>
                  )}
        ...
        {attractions.length > 0 && (
          ...
          >
            ...
            >
              ...
              <Badge
                variant="outline"
                className="self-start sm:self-auto flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-full px-3.5 py-2 shrink-0 shadow-sm"
              >
        ...
        {/* Empty state */}
        {attractions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 rounded-[26px] border border-dashed border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900">
            <span className="flex items-center justify-center w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 mb-3">
              <MapPin className="w-6 h-6 text-slate-300 dark:text-slate-600" />
            </span>
            <p className="text-sm text-slate-400 dark:text-slate-500 font-medium">
              No nearby attractions available yet.
            </p>
          </div>
        )}
      </div>
  ...
  if (!placeImages.length) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 gap-3">
        <span className="flex items-center justify-center w-14 h-14 rounded-full bg-white dark:bg-slate-900 shadow-sm">
  ...
  return (
    <div className="flex w-full h-full gap-2 p-2">
      ...
      <div className="relative flex-1 overflow-hidden rounded-2xl group">
        ...
        <span
          className="absolute bottom-3.5 left-3.5 inline-flex items-center gap-1.5 text-white text-xs font-bold
...
const AttractionCard = ({ place, index }) => (
  <motion.div
    ...
    className="group shrink-0 w-[260px] sm:w-[290px] flex flex-col bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800
    ...
    <div className="flex flex-col flex-1 p-4 gap-3">
      ...
      {(place?.distance || place?.bestTime) && (
        <div className="flex flex-wrap gap-1.5">
          {place?.distance && (
            <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-orange-600 bg-orange-50 dark:bg-orange-500/10 rounded-full px-2.5 py-1">
              <Navigation className="w-3 h-3 shrink-0" />
              {place.distance}
            </span>
          )}
          {place?.bestTime && (
            <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-sky-600 bg-sky-50 dark:bg-sky-500/10 rounded-full px-2.5 py-1">
      ...
      {/* Description */}
      {place?.description && (
        <p className="text-[12.5px] text-slate-500 dark:text-slate-400 leading-[1.6] line-clamp-2">
          {place.description}
        </p>
      )}
          ...
          {place.highlights.slice(0, 3).map((h, i) => (
            <span
              ...
              className="inline-flex items-center gap-1 text-[10.5px] font-semibold
                         bg-amber-50 dark:bg-amber-500/10 text-amber-700 ring-1 ring-amber-100
